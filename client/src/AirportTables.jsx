...
export function AirportTables({jsonSample}) {
    ...
    useEffect(() => {
        ...
        function computeCarrierSummary(key, label) {
            const flights = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_flights || 0) : t, 0);
            const delays = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_del15 || 0) : t, 0);
            const delayTime = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_delay || 0) : t, 0);
            const late = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.late_aircraft_ct || 0) : t, 0);
            const lateDelay = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.late_aircraft_delay || 0) : t, 0);
            const cancel = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_cancelled || 0) : t, 0);
            const diverted = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_diverted || 0) : t, 0);
            const carrierIssue = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.carrier_ct || 0) : t, 0);
            const carrierIssueTime = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.carrier_delay || 0) : t, 0);
    ...
    <tr>
      ...
      <th>Avg Minutes / Delayed Flight</th>
        ...
        function summarizeCarrierCounts(key, label) {
            const flights = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_flights || 0) : t, 0);
            const delayFlights = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_del15 || 0) : t, 0);
            const lateFlights = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.late_aircraft_ct || 0) : t, 0);
            const cancelled = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_cancelled || 0) : t, 0);
            const diverted = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_diverted || 0) : t, 0);
            const byCarrier = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.carrier_ct || 0) : t, 0);
            const byWeather = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.weather_ct || 0) : t, 0);
            const byNas = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.nas_ct || 0) : t, 0);
            const bySecurity = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.security_ct || 0) : t, 0);
            const byLate = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.late_aircraft_ct || 0) : t, 0);
    ...
    <td>${byLate}</td>
        ...
        function summarizeCarrierMinutes(key, label) {
            const totalMin = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_delay || 0) : t, 0);
            const carrierMin = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.carrier_delay || 0) : t, 0);
            const weatherMin = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.weather_delay || 0) : t, 0);
            const nasMin = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.nas_delay || 0) : t, 0);
            const securityMin = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.security_delay || 0) : t, 0);
            const lateMin = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.late_aircraft_delay || 0) : t, 0);
            const delayedFlt = jsonSample.current.reduce((t, r) => r.carrier === key ? t + (r.arr_del15 || 0) : t, 0);
  ...
  ` + htmlString + `</table>`;
        ...
        function displayByFlightNum() {
            ...
            let filteredArr = jsonSample.current.filter((row) => {
                return row.arr_flights >= flightNumBegin.value && row.arr_flights <= flightNumEnd.value;
            });
        ...
        <>
            ...
            <div id="carrierView" className="box">
                ...
                <div className="toolRow">
                    ...
                    <select name="carrierAscension" id="carrierAscension">
            ...
            <div id="delayReasonsView" className="box">
                ...
                <select name="delayReasonsSelect" id="delayReasonsSelect">
                    <option value="allDelays" selected>All Data</option>
                ...
                <div id="delayAirportCheckboxes">
                    <h3>Airports</h3>
                    <input type="checkbox" checked="true" id="delayAirportSelectAll" name="delayAirportSelectAll"></input>
                    <label for="delayAirportSelectAll">Select All</label> <br />
                </div>
            ...
            <div id="flightNumView" className="box">
                ...
                <div className="toolRow">
                    ...
                    to
                    <input type="number" name="flightNumEnd" id="flightNumEnd" min="0" step="10" value="10000" />
